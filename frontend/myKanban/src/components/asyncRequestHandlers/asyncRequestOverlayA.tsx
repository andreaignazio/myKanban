...
export const AsyncRequestOverlayA = ({ requestKey, minLoadingMs, minSuccessMs, maxErrorMs, variant = "overlay", show, loadingStyle = "spinner", easeIn = true }: AsyncRequestOverlayAProps) => {
    ...
    const fadeClass = `transition-opacity duration-500 ${easeIn ? "ease-in-out" : "ease-out"}`;

    if (variant === "banner") {
        return (
            <div className={`absolute inset-0 flex items-center px-3 gap-2 bg-menusec z-10 ${fadeClass} ${isVisible ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
                <div className={`absolute inset-0 flex items-center gap-2 px-3 text-yellow-500 ${fadeClass} ${showLoading && displayLoading ? "opacity-100" : "opacity-0"}`}>
                    {loadingStyle === "pulse"
                ...
                <div className={`absolute inset-0 flex items-center gap-2 px-3 text-green-500 ${fadeClass} ${showSuccess && displaySuccess ? "opacity-100" : "opacity-0"}`}>
                    <CircleCheck className="h-4 w-4 shrink-0" />
                    <span className="text-sm font-semibold truncate">Done</span>
                </div>
                <div className={`absolute inset-0 flex items-center gap-2 px-3 text-red-500 ${fadeClass} ${showError && displayError ? "opacity-100" : "opacity-0"}`}>
 ...
 */
export const FetchOverlay = ({ requestKey, minLoadingMs = 0, minSuccessMs = 0, maxErrorMs, variant = "overlay", easeIn = true, show = ["loading", "error"] }: FetchOverlayProps) => (
